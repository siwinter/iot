...
/* ****************************************************************************************
 * 
 * Die Datei beruht auf Version 0.3.4 der Library SHT31.
 * (s. https://www.arduino.cc/reference/en/libraries/sht31/)
 * 
 * Deren h- und cpp-Datei wurde einfach hintereinander in diese Datei kopiert.
 * An Ende befindet sich dann die eigen Klasse cSHT30 und die zugehörige Factory-Klasse.
 * Konstruktor und onTimeout der Klasse cSHT30 
 * 
 * ToDo: den ganzen Overhead ungenutzter Funktionen entfernen und die verbleibenden in die 
 * Klasse cSHT30 überführen. 
 * 
 * Das Modul SHT30 nutzt den i2c-Bus. Seine Adresse ist 0x45.
 * 
...
class cSHT30 : public cDevice, public cTimer {
  private :
	SHT31 sht;
	cDevice temp ;
	cDevice humid ;
	int interval ; // not less than 2 secs
  ...
  public :
	...
	void onTimeout() {
		...
		temp.setValue(sht.getTemperature() * 10) ;
		humid.setValue(sht.getHumidity()) ;}
				
	cDevice * getTemperatureSensor() {return &temp ; }
	cDevice * getHumiditySensor() {return &humid ; }
} ;

class cSHT30Factory : public cFactory {
  public:
	cSHT30Factory() {strcpy(name,"SHT30");}
	cSHT30* create() { Serial.println("create"); return new cSHT30();}
	cSHT30* create(cb_function  fTmp, cb_function  fHum) {
		cSHT30* d = create();
		new cCallBackAdapter(fTmp, d->getTemperatureSensor()) ;
		new cCallBackAdapter(fHum, d->getHumiditySensor()) ;
		return d ; }
	cSHT30* create(char*  n) {
		cSHT30* d = create();
		char txt[20] ;
		strcpy(txt, "tmp");
		strcat(txt, n) ;
